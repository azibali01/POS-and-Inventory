...
export default function SalesReturn() {
  ...
  const [invoiceNo, setInvoiceNo] = useState(() => {
    const last = localStorage.getItem("lastSalesReturnNo");
    let next = 1;
    if (last && last.startsWith("SR-")) {
      next = parseInt(last.replace("SR-", "")) + 1;
    }
    return `SR-${next.toString().padStart(4, "0")}`;
  });
  ...
  const handleSave = () => {
    ...
    const nextNo = parseInt(invoiceNo.replace("SR-", "")) + 1;
    const nextInvoice = `SR-${nextNo.toString().padStart(4, "0")}`;
    setInvoiceNo(nextInvoice);
    localStorage.setItem("lastSalesReturnNo", nextInvoice);
    setShowNotif(true);
    setTimeout(() => setShowNotif(false), 2000);
  };
